...
impl DeployPlanner {
    pub fn plan(
        ...
        env.insert(
            ...
            format!(
                "{}/clawz-agent:{}",
                env.get("CLAWZ_REGISTRY").map(String::as_str).unwrap_or("ghcr.io/improwyz"),
                tag
            ),
        );
        ...
        let (compose_overlay, standalone_source) = match (deployment, strategy) {
            (DeploymentChoice::Standalone, InstallStrategy::Source) => {
                ...
                if spec.rust_version.is_none() {
                    errors.push(
                        "Rust toolchain required for standalone source install.".into(),
                    );
            ...
            (DeploymentChoice::Standalone, InstallStrategy::Prebuilt | InstallStrategy::Build) => {
                warnings.push(
                    "Standalone mode with Docker images is unusual; prefer source or micro."
                        .into(),
                );
    ...
    pub fn compose_file_args(&self) -> Vec<&'static str> {
        match self.compose_overlay {
            ...
            Some(ComposeOverlay::Prebuilt) => {
                vec!["-f", "docker-compose.yml", "-f", "docker-compose.prebuilt.yml"]
            }
...
mod tests {
    ...
    fn prebuilt_micro_sets_tag_and_overlay() {
        ...
        .unwrap();
        assert_eq!(plan.env.get("CLAWZ_IMAGE_TAG").map(String::as_str), Some("v1.0.0"));
    ...
    fn standalone_source_sets_mode_and_commands() {
        ...
        .unwrap();
        assert_eq!(plan.env.get("CLAWZ_MODE").map(String::as_str), Some("standalone"));
