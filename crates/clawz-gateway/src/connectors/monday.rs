...
impl SaaSConnector for MondayConnector {
    ...
    async fn auth_url(&self, _redirect: &str) -> Result<String> {
        Err(ClawzError::Auth(
            "Monday.com uses API key authentication".into(),
        ))
    }
    ...
    async fn list_objects(&self, obj: &str, filters: &Filters) -> Result<Vec<Value>> {
        ...
        match obj {
            "boards" => {
                let query =
                    r#"query($limit: Int) { boards(limit: $limit) { id name description } }"#;
                let data = self
                    .graphql(query, serde_json::json!({ "limit": limit }))
                    .await?;
            ...
            "items" => {
                ...
                let data = self
                    .graphql(
                        query,
                        serde_json::json!({ "boardId": [board_id], "limit": limit }),
                    )
                    .await?;
    ...
    async fn create_object(&self, obj: &str, data: Value) -> Result<Value> {
        match obj {
            "boards" => {
                ...
                let result = self
                    .graphql(query, serde_json::json!({ "name": name }))
                    .await?;
            ...
            "items" => {
                ...
                let result = self
                    .graphql(
                        query,
                        serde_json::json!({ "boardId": board_id, "itemName": name }),
                    )
                    .await?;
    ...
    async fn delete_object(&self, obj: &str, id: &str) -> Result<()> {
        let (mutation, _key) = match obj {
            "boards" => (
                r#"mutation($id: ID!) { delete_board(board_id: $id) { id } }"#,
                "delete_board",
            ),
            "items" => (
                r#"mutation($id: ID!) { delete_item(item_id: $id) { id } }"#,
                "delete_item",
            ),
            _ => {
                return Err(ClawzError::Provider(format!(
    ...
    async fn execute_action(&self, action: &str, params: Value) -> Result<Value> {
        let query = match action {
            "move_item_to_group" => {
                r#"mutation($itemId: ID!, $groupId: String!) { move_item_to_group(item_id: $itemId, group_id: $groupId) { id } }"#
            }
            "duplicate_board" => {
                r#"mutation($boardId: ID!) { duplicate_board(board_id: $boardId, duplicate_type: duplicate_board_with_structure) { board { id name } } }"#
            }
            _ => {
                return Err(ClawzError::Provider(format!(
                    "Unknown Monday.com action: {action}"
                )));
            }
        };
