...
use clawz_gateway::{auth::api_key::ApiKeyValidator, bootstrap, server::GatewayServer, AppState};
...
async fn test_cloud_providers_list() {
    ...
    assert!(providers.len() >= 15, "expected deploy provider registry including opentofu");
}
...
async fn test_create_room_and_send_message_returns_202() {
    ...
    let agent_json: serde_json::Value =
        serde_json::from_slice(&axum::body::to_bytes(agent_resp.into_body(), usize::MAX).await.unwrap()).unwrap();
    ...
    let room_json: serde_json::Value =
        serde_json::from_slice(&axum::body::to_bytes(room_resp.into_body(), usize::MAX).await.unwrap()).unwrap();
    ...
    let msg_json: serde_json::Value =
        serde_json::from_slice(&axum::body::to_bytes(msg_resp.into_body(), usize::MAX).await.unwrap()).unwrap();
    assert_eq!(msg_json["status"], "turn_queued");
    assert!(msg_json["message"]["seq"].as_u64().unwrap_or(0) >= 1);

    let list_resp = server.app()
        .oneshot(
            Request::builder()
                .uri(format!("/api/v1/rooms/{room_id}/messages"))
                .body(Body::empty())
...
async fn test_channel_tenant_isolation() {
    ...
    let list_json: serde_json::Value =
        serde_json::from_slice(&axum::body::to_bytes(tenant_b_list.into_body(), usize::MAX).await.unwrap()).unwrap();
...
async fn test_create_conversation_links_direct_room() {
    ...
    let agent_resp = server.app().oneshot(
        Request::builder()
            .method("POST")
            .uri("/api/v1/agents")
            .header("content-type", "application/json")
    ...
    let conv_resp = server.app().oneshot(
        Request::builder()
            .method("POST")
            .uri("/api/v1/conversations")
            .header("content-type", "application/json")
    ...
    let conv_json: serde_json::Value =
        serde_json::from_slice(&axum::body::to_bytes(conv_resp.into_body(), usize::MAX).await.unwrap()).unwrap();
    let conv_id = conv_json["id"].as_str().unwrap();
    assert_eq!(conv_json["room_id"], conv_id);

    let room_resp = server.app().oneshot(
        Request::builder()
...
async fn test_conversation_send_message_uses_room_pipeline() {
    ...
    let msg_resp = server.app().oneshot(
        Request::builder()
            .method("POST")
    ...
    let list_resp = server.app().oneshot(
        Request::builder()
            .uri(format!("/api/v1/conversations/{conv_id}/messages"))
            .body(Body::empty())
            .unwrap(),
...
async fn test_create_channel_roundtrip() {
    ...
    let created = server.app().oneshot(
        Request::builder()
            .method("POST")
            .uri("/api/v1/channels")
            .header("content-type", "application/json")
    ...
    let channel: serde_json::Value =
        serde_json::from_slice(&axum::body::to_bytes(created.into_body(), usize::MAX).await.unwrap()).unwrap();
    let channel_id = channel["id"].as_str().unwrap();

    let fetched = server.app().oneshot(
        Request::builder()
            .uri(format!("/api/v1/channels/{channel_id}"))
            .body(Body::empty())
            .unwrap(),
